...
if uploaded:
    ...
    try:
        ...
        report = ReportService().stakeholder_report(df)
        ...
        with tabs[1]:
            ...
            st.plotly_chart(dupont_component_chart(analysis["dupont"]), use_container_width=True)
        with tabs[2]:
        ...
        with tabs[3]:
            st.plotly_chart(trend_chart(df, ["revenue", "gross_profit", "operating_profit", "net_profit", "total_assets", "total_liabilities", "equity", "operating_cash_flow", "investing_cash_flow", "financing_cash_flow"]), use_container_width=True)
            st.plotly_chart(red_flag_chart(analysis["red_flags"] if isinstance(analysis["red_flags"], list) else []), use_container_width=True)
        with tabs[4]:
        ...
        with tabs[6]:
            st.json(report)
            st.download_button("Download HTML report", export_html(report), file_name="stakeholder_report.html")
