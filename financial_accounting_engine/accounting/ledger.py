...
def post_to_ledgers(journal: pd.DataFrame, accounts: dict[str, Account]) -> pd.DataFrame:
    ...
    for row in _sort_journal_by_posting_date(journal).itertuples(index=False):
...
def _sort_journal_by_posting_date(journal: pd.DataFrame) -> pd.DataFrame:
    return (
        journal.assign(posting_datetime=pd.to_datetime(journal["date"], errors="raise"))
        .sort_values(["posting_datetime"], kind="mergesort")
        .drop(columns=["posting_datetime"])
    )
