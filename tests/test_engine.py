...
from financial_accounting_engine.forecasting.baseline_models import cagr_forecast
...
from financial_accounting_engine.services.forecasting_service import (
    ForecastingService,
...
def test_journal_totals_reconcile_and_mismatch_is_flagged():
    result = run_accounting_cycle(detailed_transactions(), detailed_opening_balances())
    journal = result["journal"]
    assert isinstance(journal, pd.DataFrame)

    reconciliation = reconcile_journal_totals(journal)
    mismatched = journal.copy()
    mismatched.loc[0, "credit_amount"] = 1
    mismatch_reconciliation = reconcile_journal_totals(mismatched)

    assert result["journal_reconciliation"] == reconciliation
    assert reconciliation["balanced"] is True
    assert reconciliation["difference"] == 0
    assert mismatch_reconciliation["balanced"] is False
    assert mismatch_reconciliation["difference"] == 199


def test_ledger_final_balances_reconcile_to_trial_balance_and_mismatch_is_flagged():
    result = run_accounting_cycle(detailed_transactions(), detailed_opening_balances())
    ledger = result["ledger"]
    trial_balance = result["trial_balance"]
    assert isinstance(ledger, pd.DataFrame)
    ...
    reconciliation = reconcile_ledger_to_trial_balance(ledger, trial_balance)
    mismatched_trial_balance = trial_balance.copy()
    mismatched_trial_balance.loc[mismatched_trial_balance.account_name == "Cash", "debit_balance"] += 25
    mismatch_reconciliation = reconcile_ledger_to_trial_balance(ledger, mismatched_trial_balance)

    assert result["ledger_trial_balance_reconciliation"] == reconciliation
    assert reconciliation["balanced"] is True
    assert reconciliation["account_count"] == len(detailed_opening_balances())
    assert mismatch_reconciliation["balanced"] is False
    assert mismatch_reconciliation["differences"][0]["account_name"] == "Cash"


def test_balance_sheet_reconciliation_paths():
    balanced = reconcile_balance_sheet(total_assets=200, total_liabilities=50, equity=150)
    mismatched = reconcile_balance_sheet(total_assets=200, total_liabilities=60, equity=150)

    assert balanced["balanced"] is True
    assert balanced["difference"] == 0
    assert mismatched["balanced"] is False
    assert mismatched["difference"] == -10


def test_cash_flow_reconciliation_paths():
    balanced = reconcile_cash_flow(opening_cash=10, operating=20, investing=-5, financing=15, closing_cash=40)
    mismatched = reconcile_cash_flow(opening_cash=10, operating=20, investing=-5, financing=15, closing_cash=35)

    assert balanced["balanced"] is True
    assert balanced["difference"] == 0
    assert mismatched["balanced"] is False
    assert mismatched["difference"] == 5


def test_final_accounts_connect_profit_depreciation_capital_drawings_and_balance_sheet():
    final = build_final_accounts(
        {"opening_stock": 0, "purchases": 100, "purchase_returns": 0, "direct_expenses": 0, "sales": 200, "sales_returns": 0, "closing_stock": 0},
        {"indirect_income": 0, "admin_expenses": 20, "selling_expenses": 0, "distribution_expenses": 0, "finance_costs": 0, "depreciation": 10, "bad_debts": 0, "provisions": 0, "other_operating_expenses": 0},
        {"non_current_assets": 160, "current_assets": 40, "capital": 100, "drawings": 20, "current_liabilities": 50, "non_current_liabilities": 0},
    )

    assert final["trading_account"]["gross_profit"] == 100
    assert final["profit_and_loss"]["depreciation"] == 10
    assert final["profit_and_loss"]["net_profit"] == 70
    assert final["balance_sheet"]["equity"] == 150
    assert final["balance_sheet"]["balanced"] is True


def test_ratio_zero_denominators_return_clear_warnings_for_key_ratio_groups():
    df = statements().head(1).copy()
    df.loc[0, ["revenue", "current_liabilities", "equity", "total_assets", "interest_expense", "COGS"]] = 0

    ratios = calculate_ratios(df)
    warned = ratios[ratios.warning == "zero_or_invalid_denominator"]

    assert not warned.empty
    assert {"current_ratio", "debt_equity", "ROA", "interest_coverage", "gross_profit_ratio"}.issubset(set(warned.ratio_name))
    assert warned["interpretation"].str.contains("denominator is zero or invalid").all()


def test_streamlit_home_uses_services_for_visualization_and_report_exports():
    app_source = Path("financial_accounting_engine/app/streamlit_app.py").read_text()

    assert "from financial_accounting_engine.services.visualization_service import VisualizationService" in app_source
    assert "from financial_accounting_engine.services.report_service import ReportService" in app_source
    assert "from financial_accounting_engine.visualization." not in app_source
    assert "from financial_accounting_engine.reports." not in app_source


def _zero_revenue_statements(current_revenue: float, previous_revenue: float) -> pd.DataFrame:
    df = statements().head(2).copy()
    df.loc[0, "revenue"] = previous_revenue
    df.loc[1, "revenue"] = current_revenue
    return df


def test_red_flags_current_period_zero_revenue_returns_warning_not_crash():
    flags = detect_red_flags(_zero_revenue_statements(current_revenue=0, previous_revenue=100))

    assert any(flag["flag"] == "Zero revenue margin comparison skipped" for flag in flags)


def test_red_flags_previous_period_zero_revenue_returns_warning_not_crash():
    flags = detect_red_flags(_zero_revenue_statements(current_revenue=100, previous_revenue=0))

    assert any(flag["flag"] == "Zero revenue margin comparison skipped" for flag in flags)


def test_red_flags_both_periods_zero_revenue_returns_warning_not_crash():
    flags = detect_red_flags(_zero_revenue_statements(current_revenue=0, previous_revenue=0))

    warning_flags = [flag for flag in flags if flag["flag"] == "Zero revenue margin comparison skipped"]
    assert warning_flags
    assert "non-zero current and previous revenue" in warning_flags[0]["detail"]


def test_analysis_service_handles_zero_revenue_without_crashing():
    analysis = AnalysisService().analyze(_zero_revenue_statements(current_revenue=0, previous_revenue=0))

    assert "red_flags" in analysis
    assert any(flag["flag"] == "Zero revenue margin comparison skipped" for flag in analysis["red_flags"])


def test_forecast_periods_one_returns_one_row_per_metric():
    forecast = ForecastingService().forecast(statements(), periods=1)
    revenue_forecast = forecast[forecast.metric == "revenue"]

    assert len(revenue_forecast) == 1
    assert revenue_forecast.iloc[0].period == "T+1"


def test_forecast_periods_three_returns_three_aligned_rows_per_metric():
    forecast = ForecastingService().forecast(statements(), periods=3)
    revenue_forecast = forecast[forecast.metric == "revenue"]

    assert len(revenue_forecast) == 3
    assert revenue_forecast["period"].tolist() == ["T+1", "T+2", "T+3"]
    assert revenue_forecast[["expected", "lower", "upper"]].notna().all().all()


def test_cagr_forecast_changes_over_multi_period_growth_horizon():
    forecast_values = cagr_forecast(pd.Series([100.0, 110.0, 121.0]), periods=3)

    assert len(forecast_values) == 3
    assert forecast_values[0] < forecast_values[1] < forecast_values[2]


def test_prediction_intervals_align_one_to_one_with_forecast_values():
    forecast_values = cagr_forecast(pd.Series([100.0, 110.0, 121.0]), periods=3)
    intervals = residual_interval(pd.Series([100.0, 110.0, 121.0]), forecast_values)

    assert len(intervals) == len(forecast_values)
    assert [interval["expected"] for interval in intervals] == forecast_values


def test_forecast_explanations_do_not_claim_exact_certainty():
    forecast = ForecastingService().forecast(statements(), periods=3)

    assert forecast["explanation"].str.contains("estimates, not guarantees").all()
    assert not forecast["explanation"].str.contains("guaranteed|certain|exact", case=False).any()


def pro_forma_assumptions(**overrides) -> ProFormaAssumptions:
    values = {
        "revenue_growth_rate": 0.10,
        "cogs_percentage_of_revenue": 0.50,
        "operating_expense_percentage_of_revenue": 0.20,
        "depreciation_amount": 5.0,
        "interest_expense": 2.0,
        "tax_rate": 0.25,
        "receivables_days": 30.0,
        "inventory_days": 20.0,
        "payables_days": 25.0,
        "capex": 10.0,
        "debt_change": 0.0,
    ...
    return ProFormaAssumptions(**values)


def test_pro_forma_income_statement_calculations():
    result = ProFormaService().project(statements(), pro_forma_assumptions())
    income = result.income_statement.iloc[0]

    assert income.revenue == 154
    assert income.COGS == 77
    assert income.gross_profit == 77
    assert income.operating_expenses == 30.8
    assert income.operating_profit == 41.2
    assert income.tax_expense == 9.8
    assert income.net_profit == 29.400000000000002


def test_pro_forma_balance_sheet_reconciliation_success():
    result = ProFormaService().project(statements(), pro_forma_assumptions())
    balance = result.balance_sheet.iloc[0]

    assert bool(balance.balanced) is True
    assert abs(balance.balancing_difference) <= 0.01
    assert result.reconciliation["all_balanced"] is True


def test_pro_forma_balance_sheet_mismatch_warning_without_auto_balance():
    result = ProFormaService().project(statements(), pro_forma_assumptions(auto_balance=False))

    assert result.reconciliation["all_balanced"] is False
    assert any(warning["risk"] == "balance_sheet_mismatch" for warning in result.risk_warnings)


def test_pro_forma_cash_flow_reconciliation_success():
    result = ProFormaService().project(statements(), pro_forma_assumptions())
    cash_flow = result.cash_flow_statement.iloc[0]

    assert bool(cash_flow.balanced) is True
    assert abs(cash_flow.opening_cash + cash_flow.operating_cash_flow + cash_flow.investing_cash_flow + cash_flow.financing_cash_flow - cash_flow.closing_cash) <= 0.01


def test_pro_forma_cash_flow_mismatch_warning_from_reconciliation_rows():
    result = ProFormaService().project(statements(), pro_forma_assumptions(auto_balance=False))
    period_reconciliation = result.reconciliation["periods"][0]
    period_reconciliation["cash_flow"]["balanced"] = False
    warnings = __import__("financial_accounting_engine.statements.pro_forma", fromlist=["scenario_risk_warnings"]).scenario_risk_warnings(
        result.income_statement.merge(result.balance_sheet, on="period").merge(result.cash_flow_statement, on="period"),
        result.ratios,
        [period_reconciliation],
    )

    assert any(warning["risk"] == "cash_flow_mismatch" for warning in warnings)


def test_pro_forma_forecasted_ratio_generation():
    result = ProFormaService().project(statements(), pro_forma_assumptions())

    assert {"current_ratio", "quick_ratio", "debt_equity", "interest_coverage", "ROA", "ROE"}.issubset(set(result.ratios.ratio_name))


def test_pro_forma_generates_four_default_scenarios():
    scenarios = ProFormaService().scenarios(statements(), forecast_periods=2)

    assert set(scenarios) == {"base", "optimistic", "pessimistic", "stress"}
    assert all(len(scenario.pro_forma.income_statement) == 2 for scenario in scenarios.values())
    assert all(scenario.explanation for scenario in scenarios.values())


def test_pro_forma_negative_cash_warning():
    result = ProFormaService().project(statements(), pro_forma_assumptions(capex=1000, auto_balance=False))

    assert any(warning["risk"] == "negative_closing_cash" for warning in result.risk_warnings)


def test_pro_forma_negative_working_capital_warning():
    result = ProFormaService().project(statements(), pro_forma_assumptions(receivables_days=0, inventory_days=0, payables_days=400, auto_balance=False))

    assert any(warning["risk"] == "negative_working_capital" for warning in result.risk_warnings)


def test_pro_forma_high_debt_equity_warning():
    result = ProFormaService().project(statements(), pro_forma_assumptions(debt_change=500, auto_balance=False))

    assert any(warning["risk"] == "high_debt_equity" for warning in result.risk_warnings)


def test_pro_forma_low_interest_coverage_warning():
    result = ProFormaService().project(statements(), pro_forma_assumptions(interest_expense=100, auto_balance=False))

    assert any(warning["risk"] == "low_interest_coverage" for warning in result.risk_warnings)


def test_pro_forma_service_output_shape():
    result = ProFormaService().project(statements(), pro_forma_assumptions(forecast_periods=3))

    assert len(result.income_statement) == 3
    assert len(result.balance_sheet) == 3
    assert len(result.cash_flow_statement) == 3
    assert not result.ratios.empty
    assert isinstance(result.reconciliation, dict)
    assert isinstance(result.risk_warnings, list)


def test_pro_forma_zero_revenue_and_zero_denominator_safeguards():
    zero = statements().tail(1).copy()
    zero.loc[zero.index[0], "revenue"] = 0
    zero.loc[zero.index[0], "current_liabilities"] = 0
    result = ProFormaService().project(zero, pro_forma_assumptions(revenue_growth_rate=0, cogs_percentage_of_revenue=0, operating_expense_percentage_of_revenue=0))

    warned = result.ratios[result.ratios.warning == "zero_or_invalid_denominator"]
    assert result.income_statement.iloc[0].revenue == 0
    assert not warned.empty


def non_iso_opening_balances() -> pd.DataFrame:
    return pd.DataFrame([
        {"account_name": "Cash", "account_type": "asset", "opening_debit_balance": 1000, "opening_credit_balance": 0, "period_start_date": "2024-01-01"},
        {"account_name": "Capital", "account_type": "capital", "opening_debit_balance": 0, "opening_credit_balance": 1000, "period_start_date": "2024-01-01"},
        {"account_name": "Sales", "account_type": "revenue", "opening_debit_balance": 0, "opening_credit_balance": 0, "period_start_date": "2024-01-01"},
        {"account_name": "Rent Expense", "account_type": "expense", "opening_debit_balance": 0, "opening_credit_balance": 0, "period_start_date": "2024-01-01"},
    ])


def test_ledger_sorts_non_iso_dates_by_parsed_datetime_not_lexicographically():
    tx = pd.DataFrame([
        {"transaction_id": "T1", "date": "1/10/2024", "description": "Cash sale", "debit_account": "Cash", "credit_account": "Sales", "amount": 100, "currency": "USD", "category": "sales", "source_document": "INV", "narration": "sale"},
        {"transaction_id": "T2", "date": "1/2/2024", "description": "Rent paid", "debit_account": "Rent Expense", "credit_account": "Cash", "amount": 25, "currency": "USD", "category": "rent", "source_document": "RCPT", "narration": "rent"},
    ])

    result = run_accounting_cycle(tx, non_iso_opening_balances())
    ledger = result["ledger"]
    cash_rows = ledger[ledger.account_name == "Cash"].reset_index(drop=True)
    cash_trial = result["trial_balance"][result["trial_balance"].account_name == "Cash"].iloc[0]

    assert cash_rows.loc[1, "date"] == "1/2/2024"
    assert cash_rows.loc[1, "running_balance"] == 975
    assert cash_rows.loc[2, "date"] == "1/10/2024"
    assert cash_rows.loc[2, "running_balance"] == 1075
    assert cash_trial.debit_balance == 1075


def test_forecast_derived_quick_ratio_excludes_inventory_and_prepaids():
    df = statements().head(1).copy()
    df["inventory"] = 30
    df["prepaid_expenses"] = 5

    derived = _with_derived_metrics(df)

    assert derived.loc[0, "current_ratio"] == 2
    assert derived.loc[0, "quick_ratio"] == 0.6


def test_forecasting_skips_all_missing_derived_metrics_without_crashing():
    df = statements().copy()
    df["equity"] = 0

    forecast = ForecastingService().forecast(df, periods=2)
    skipped = forecast[forecast.model == "not_forecastable"]

    assert {"debt_equity", "ROE"}.issubset(set(skipped.metric))
    assert skipped["warning"].str.contains("no usable historical observations").all()
    assert not forecast[forecast.metric == "revenue"].empty
